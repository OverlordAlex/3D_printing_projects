...
//       in the 'hinge_leaf' module to 'true', which will enable a single row of inscribed text on each hinge_leaf.
...
C_MIN_hinge_leaf_GAUGE                           = C_CONSTANT + 1.0;
...
enable_male_hinge_leaf             = 1;     // [ 0:No, 1:Yes ]
enable_female_hinge_leaf           = 1;     // [ 0:No, 1:Yes ]
enable_fillet                = 1;     // [ 0:No, 1:Yes ]
// Turn this off to omit the hinge pin from the female hinge_leaf.
enable_pin                   = 1;     // [ 0:No, 1:Yes ]
// Turn this off to set a custom pin diameter. Auto pin size is equal to the hinge_leaf gauge.
enable_auto_pin_size         = 1;     // [ 0:No, 1:Yes ]
enable_pin_shaft_counterbore = 0;     // [ 0:No, 1:Yes ]
enable_fasteners             = 0;     // [ 0:No, 1:Yes ]
knuckle_gusset_type          = 1;     // [ 0:None, 1:Linear, 2:Circular, 3:Parabolic ]
...
pin_diameter                          = 3.0;
parametric_pin_diameter               = ( enable_auto_pin_size == 1 ) ? hinge_leaf_gauge : pin_diameter;
...
m_male_hinge_leaf_enabled             = ( enable_male_hinge_leaf              == 1 ) ? true : false;
m_female_hinge_leaf_enabled           = ( enable_female_hinge_leaf            == 1 ) ? true : false;
m_hinge_leaf_fillet_enabled           = ( enable_fillet                 == 1 ) ? true : false;
...
// hinge_leaf Parameters.

m_hinge_width = ( hinge_width <= C_MIN_HINGE_WIDTH ) ? C_MIN_HINGE_WIDTH : hinge_width;
m_hinge_leaf_width  = m_hinge_width / 2.0;
m_hinge_leaf_height = ( hinge_leaf_height <= C_MIN_HINGE_HEIGHT ) ? C_MIN_HINGE_HEIGHT : hinge_leaf_height;
m_hinge_leaf_gauge  = clip ( hinge_leaf_gauge, C_MIN_hinge_leaf_GAUGE, m_hinge_leaf_width/2.0 );

// Mechanical Properties.

m_component_clearance  = clip ( component_clearance, C_MIN_COMPONENT_CLEARENCE, C_MAX_COMPONENT_CLEARENCE );
m_knuckle_outer_radius = m_hinge_leaf_gauge * 2.0;
m_knuckle_count        = clip ( 1 + 2 * floor ( knuckle_count / 2 ), C_MIN_KNUCKLE_COUNT, C_MAX_KNUCKLE_COUNT );
m_fastener_margin      = clip ( fastener_margin, C_MIN_FASTENER_MARGIN, ( m_hinge_leaf_width - m_hinge_leaf_gauge - fastener_head_diameter )/2.0  );
m_knuckle_gusset_width = clip ( m_fastener_margin, C_MIN_KNUCKLE_GUSSET_WIDTH, m_hinge_leaf_width - m_hinge_leaf_gauge - m_component_clearance );

// Pin Parameters.

m_pin_diameter                          = clip ( pin_diameter, C_MIN_PIN_DIAMETER, 2.0*m_hinge_leaf_gauge - C_MIN_PIN_DIAMETER );
m_parametric_pin_diameter               = ( m_pin_auto_size_enabled == true ) ? m_hinge_leaf_gauge : m_pin_diameter;
m_top_pin_shaft_counterbore_diameter    = clip ( top_pin_shaft_counterbore_diameter, C_MIN_PIN_SHAFT_COUNTERBORE_DIAMETER, 2.0*m_hinge_leaf_gauge - C_MIN_PIN_SHAFT_COUNTERBORE_WALL_THICKNESS );
m_top_pin_shaft_counterbore_depth       = clip ( top_pin_shaft_counterbore_depth, C_MIN_PIN_SHAFT_COUNTERBORE_DEPTH, 0.66*( m_hinge_leaf_height / ( m_knuckle_count + m_component_clearance )) );
m_top_pin_shaft_counterbore_shape       = top_pin_shaft_counterbore_shape;
m_bottom_pin_shaft_counterbore_diameter = clip ( bottom_pin_shaft_counterbore_diameter, C_MIN_PIN_SHAFT_COUNTERBORE_DIAMETER, 2.0*m_hinge_leaf_gauge - C_MIN_PIN_SHAFT_COUNTERBORE_WALL_THICKNESS );
m_bottom_pin_shaft_counterbore_depth    = clip ( bottom_pin_shaft_counterbore_depth, C_MIN_PIN_SHAFT_COUNTERBORE_DEPTH, 0.8*( m_hinge_leaf_height / m_knuckle_count) );
m_bottom_pin_shaft_counterbore_shape    = bottom_pin_shaft_counterbore_shape;

// Fastener parameters.

m_fstener_head_type        = fstener_head_type;
m_counter_sink_depth       = clip ( counter_sink_depth, 0.0, m_hinge_leaf_gauge - C_MIN_COUNTER_SINK_DEPTH_STOP );
m_fastener_head_diameter   = clip ( fastener_head_diameter, fastener_thread_diameter, m_hinge_leaf_width - m_hinge_leaf_gauge - m_component_clearance - 2.0*m_fastener_margin );
...
m_fastener_count           = clip ( fastener_count, C_MIN_FASTENER_COUNT, m_fastener_column_count*( m_hinge_leaf_height - 2.0*m_fastener_margin )/(m_fastener_head_diameter + m_component_clearance));
m_hinge_leaf_fillet_radius       = m_fastener_head_diameter / 2.0 + m_fastener_margin;
...
if ( C_DEBUG_ENABLED )
{
    ...
    echo ( m_hinge_leaf_width                     = m_hinge_leaf_width );
    echo ( absolute_hinge_leaf_width              = m_hinge_leaf_width - m_hinge_leaf_gauge );
    echo ( m_hinge_leaf_height                    = m_hinge_leaf_height );
    echo ( m_hinge_leaf_gauge                     = m_hinge_leaf_gauge );
    ...
    echo ( m_hinge_leaf_fillet_radius             = m_hinge_leaf_fillet_radius );
...
// Description:
//
// - Creates a hinge hinge_leaf component, whose gender may be selected through the gender argument.
//
// - Note:
...
//
// - gender:
//   The gender (male, female), of the hinge_leaf. The female hinge_leaf holds the pin.
//
// -------------------------------------+---------------------------------------+---------------------------------------+---------------------------------------

module hinge_leaf ( gender )
{  
    // Text configuration.
    ...
    // - In other words, we leave the female hinge_leaf un-rotated, but we rotate the male hinge_leaf 180 degrees, to place it at an 
    ...
    rotate ( [ 0, 0, gender_angle ] )
    {
        ...
        difference ()
        {
            ...
            workpiece_hinge_leaf_knuckle_pin ( gender );   
            ...
            // Cut pin shaft counterbore into female hinge_leaf.
            ...
            // Cut text.
            // -  We will only cut text into the leaves, if we are using exactly 4 fasteners per hinge_leaf.
            // -  All other hinge_leaf counts will not leave enough space for the text to fit easily. So we only add text, if we are using 4 fasteners.
...
// -------------------------------------+---------------------------------------+---------------------------------------+---------------------------------------
// Module:      workpiece_hinge_leaf_knuckle_pin
...
module workpiece_hinge_leaf_knuckle_pin ( gender )
{   
    ...
    // Combine pin with hinge_leaf and knuckle.
    
    if ( gender == C_FEMALE )
    {
        if ( m_pin_enabled )
        {
            // Fuse the pin to the female hinge_leaf by default.
            
            union ()
            {                           
                dc = d - c/2.0;
                
                workpiece_hinge_leaf_knuckle ( C_FEMALE );            
                pin ( dc, m_hinge_leaf_height - e );
            }
        }
        else
        {
    ...
    {
        // Cut a hole for the pin to pass throug in the male hinge_leaf.
        
        difference ()
        {   
            dc = d + c/2.0;            
            
            workpiece_hinge_leaf_knuckle ( C_MALE );            
            pin ( dc, m_hinge_leaf_height + e );
        }
    }
}
...
// -------------------------------------+---------------------------------------+---------------------------------------+---------------------------------------
// Module:      workpiece_hinge_leaf_knuckle
// Module Type: Workpiece.
//
// Description:
//
...
module workpiece_hinge_leaf_knuckle ( gender )
{
    // Initialize local variables.
    ...
    difference ()
    {
        difference ()
        {
            // hinge_leaf and knuckle work piece.
            ...
            (
                ...
                size                     = 2.0*m_hinge_leaf_gauge + m_component_clearance
            );
        }
...
// -------------------------------------+---------------------------------------+---------------------------------------+---------------------------------------
// Module:      workpiece_hinge_leaf
// Module Type: Workpiece.
//
// Description:
//
// - Workpiece used to cut away hinge_leaf structures.
//
// Parameters:
//
// - w:
//   Width of a single hinge_leaf.
//
// - l:
//   Length of the hinge_leaf.
//
// - h:
//   Height of the hinge_leaf.
//
// - r:
...
module workpiece_hinge_leaf ( w, l, h, r )
{
    translate ( [ 0, 0, -h ] )
    {   
        union ()
        {
            if ( m_hinge_leaf_fillet_enabled )
            {
                // hinge_leaf.
...
//
// - gender:
//   Specifies the number of knuckle joints based on hinge hinge_leaf gender.   
//
// - scg_type:
...
{   
    ...
    cutting_block_size = m_hinge_leaf_gauge + m_knuckle_gusset_width;
    hinge_leaf_height        = ( scg_type == C_NEGATIVE ) ? m_hinge_leaf_height + SCG_OVERLAP : m_hinge_leaf_height; 
    ...
    difference ()
    {
        ...
        (
            ...
            height        = hinge_leaf_height, 
            knuckle_radus = m_hinge_leaf_gauge, 
            curve         = curve,
            scg_type      = scg_type
        );
...
//
// - knuckle_radus
//   Outer radius of the knuckles. This is also the same value as the hinge_leaf gauge.
//
// - curve
...
module workpiece_gusset_linear ( width, height, knuckle_radus, scg_type )
{
    ...
    h = height;         // Hinge hinge_leaf height.
    r = knuckle_radus;  // Knuckle radius is equal to the hinge_leaf gauge.
    s = w + r;          // Cartesian position of the point where the gusset curve merges with the hinge_leaf.
    g = m_hinge_leaf_gauge;   // hinge_leafe gauge.
...
module workpiece_gusset_circular ( width, height, knuckle_radus, scg_type )
{
    ...
    w = width;          // Gusset width.
    g = knuckle_radus;  // Knuckle radius is equal to the hinge_leaf gauge.
    c = SCG_OVERLAP;    // Amount to overlap unions in order to prevent Boolean anomalies.
...
// Description:
//
// - Cutting tool used to cut fastener holes into hinge_leaf workpiece.
//
// Parameters:
//
// - z_offset:
...
{   
    ...
    z0     = z_offset - m_hinge_leaf_gauge;               // Vertical position of head.
    z1     = z_offset - m_counter_sink_depth;       // Vertical position of thread.
    h0     = m_hinge_leaf_gauge;                          // Height of head.
    ...
    union ()
    {
        ...
        union ()
        {
            ...
            translate ( [ 0, 0, c ] )
            {   
                cylinder ( d = od, h = m_hinge_leaf_gauge );
            }
        }
    }
}
...
//
// - fastener_count:
//   Number of fastener holes to be cut into a single hinge_leaf.
//   The total number of fastener holes on the whole hinge, will be twice the value of fastener_count.
//   i.e. 'fastener_count' holes, on each hinge_leaf.
//
// - fastener_column_count:
//   Number of fastener column per hinge_leaf. This value can be either 1, or 2.
//
// - z_offset:
//   Depth of the cut.
//
...
module tool_cutter_fastener_set ( fastener_count, fastener_column_count, z_offset )
{
    ...
    xo = m_hinge_leaf_gauge + m_component_clearance/2 + m_fastener_head_diameter/2 + m_fastener_margin;
    yo = -m_hinge_leaf_height/2 + m_fastener_head_diameter/2 + m_fastener_margin;
    
    // Column offsets.
    
    col0 = 0;
    col1 = m_hinge_leaf_width - m_fastener_head_diameter/2 - m_fastener_margin - xo;
    ...
    k1   = ( m_hinge_leaf_height - m_fastener_head_diameter - 2*m_fastener_margin ) / n1;
    k2   = ( m_hinge_leaf_height - m_fastener_head_diameter - 2*m_fastener_margin ) / n2;
...
module tool_cutter_pin_shaft_counterbore ( diameter_top, depth_top, shape_top, diameter_bottom, depth_bottom, shape_bottom )
{
    ...
    xd  = 0.0;
    yd0 =  ( h0 - m_hinge_leaf_height )/2.0 - SCG_OVERLAP;
    yd1 = -( h1 - m_hinge_leaf_height )/2.0 + SCG_OVERLAP;
    zd  = 0.0;
